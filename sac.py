...
class SAC(nn.Module):
    def __init__(
        ...
        # Actor
        self.actor = Actor(action_dim, state_dim).to(device)

        # Critic
        self.critic = Dual_Critic(state_dim, action_dim).to(device)
        self.critic_target = Dual_Critic(state_dim, action_dim).to(device)
    ...
    def sample_action(self, state, deterministic=False):
        return self.actor.sample(state, deterministic)[0]
